<?php
 ...
 Class quickbase {
	...
	// Load the quickbase config file which contains sensitive project specific
	// data include API token, username, password, and database ID
 ...
 -----------------------------------------------------------------------*/
	...
	private function init_from_config() {
		...
		$this->ci->config->load('quickbase');
		$this->user_name = $this->ci->config->item('qb_user_name'); 	// QuickBase user who will access the QuickBase
		$this->passwd = $this->ci->config->item('qb_passwd');		// Password of this user
		$this->db_id = $this->ci->config->item('qb_db_id'); 		// Table/Database ID of the QuickBase being accessed
		$this->app_token = $this->ci->config->item('qb_app_token');
		$this->xml = $this->ci->config->item('qb_xml');
		$this->user_id = $this->ci->config->item('qb_user_id');
		$this->qb_site = $this->ci->config->item('qb_qb_site');
		$this->qb_ssl = $this->ci->config->item('qb_qb_ssl');
		$this->ticketHours = $this->ci->config->item('qb_ticketHours');
